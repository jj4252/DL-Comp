"""
MoCo V3 evaluation script for testset datasets
Evaluates on validation set (shows accuracy) and generates test set predictions (CSV)
Supports multiple datasets: testset_1 (CUB-200), testset_2 (MiniImageNet), testset_3 (SUN397)
"""
import re
import torch
...
from ssl_vision.models import create_vision_transformer, create_moco_model
...
def load_moco_backbone(checkpoint_path: str, device: torch.device, cfg: DictConfig):
    """Load MoCo V3 checkpoint and extract encoder_q as backbone"""
    ...
    # Create backbone directly using the config
    print("Creating backbone from config...")
    backbone = create_vision_transformer(snapped_cfg)
    
    # Load weights - extract encoder_q weights from MoCo checkpoint
    if 'student_state_dict' in checkpoint:
        # MoCo checkpoint saved during training
        ...
        # Extract encoder_q weights
        encoder_q_state_dict = {}
        for k, v in state_dict.items():
            if k.startswith('encoder_q.'):
                # Remove 'encoder_q.' prefix
                new_key = k.replace('encoder_q.', '')
                encoder_q_state_dict[new_key] = v
        
        if encoder_q_state_dict:
            backbone.load_state_dict(encoder_q_state_dict, strict=False)
            print(f"✓ Loaded encoder_q weights ({len(encoder_q_state_dict)} parameters)")
        else:
            # Fallback: try loading as if it's a direct backbone checkpoint
            print("⚠️  Warning: Could not find encoder_q weights, trying direct load...")
...
@hydra.main(version_base=None, config_path="../configs", config_name="eval_moco_submission")
def main(cfg: DictConfig):
    ...
    print(f"{dataset_name} MoCo V3 Evaluation")
    print("=" * 80)
    ...
    # Look for specific checkpoint file or all checkpoints
    checkpoint_file = checkpoint_dir / eval_cfg.get('checkpoint_file', 'checkpoint_epoch_250.pth')
    if checkpoint_file.exists():
        checkpoint_paths = [checkpoint_file]
    else:
        checkpoint_paths = sorted((p for p in checkpoint_dir.glob('*.pth')), reverse=True)
    ...
    print(f"\nFound {len(checkpoint_paths)} checkpoint(s) to evaluate")
    ...
    for checkpoint_path in checkpoint_paths:
        ...
        backbone = load_moco_backbone(str(checkpoint_path), device, cfg)
        backbone.eval()
    ...
    with open(summary_file, 'w') as f:
        f.write(f"{eval_cfg.data['0'].dataset_name} MoCo V3 Evaluation Summary\n")
        f.write(f"{'='*60}\n")
